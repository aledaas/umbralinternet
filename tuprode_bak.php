...
//bloque para saber si existe ya un prode para este usuario.-
	$sql_prode = "select count(*) from prodes where id_usuario = ".$_SESSION['id_usuario'];
...
if ($total_count == 0)
{
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		values (".$prode_id.",".$row['id_partido'].",".$_SESSION['id_usuario'].", 'A')";
...
{
	$sql_mi_prode = "select distinct id_prode from prodes where id_usuario = ".$_SESSION['id_usuario'];
	...
	and pp.id_usuario = ". $_SESSION['id_usuario']." order by id_partido";
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		echo '<input name="goleq01[]" id="'.$row['id_partido'].'" type="text" size="1" maxlength="2" value= '.$row['golequipo01'].' />';
		echo '</td><td>';
		echo '<input name="goleq02[]" id="'.$row['id_partido'].'" type="text" size="1" maxlength="2" value= '.$row['golequipo02'].' />';
